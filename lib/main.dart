...
void main() async {
  ...
  // ── 1b. Initialize offline database (Isar) — best-effort ───
  // Powers offline order-taking: local order storage + the sync outbox.
  // Like Firebase below, a failure here must never stop the POS launching —
  // the app simply falls back to online-only behaviour.
  try {
    await IsarService.instance.initialize();
  } catch (e) {
    if (kDebugMode) {
      debugPrint('[Isar] Offline DB init failed — offline mode disabled: $e');
    }
  }
